...
-- Description	: Insert template journal recurring baru.
--				  Contoh: amortisasi sewa dibayar dimuka -
--				  Debet Biaya Sewa, Kredit Sewa Dibayar Dimuka per bulan.
-- =============================================

-- EXEC [dbo].[USP_GL_RecurringJournal_Create] 1,2,'RJ-SEWA','Amortisasi sewa kantor','Sewa kantor 12 bulan',10,20,4833333.33,'202607','202706','A','it_febry','A'

CREATE PROCEDURE [dbo].[USP_GL_RecurringJournal_Create]
	@IDX_M_Company			INT,
	@IDX_M_Branch			INT,
	...
	@EndPeriod				VARCHAR(6),		-- kosong = tanpa batas akhir
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @_IDX_M_RecurringJournal AS BIGINT
		...
		BEGIN
			...
			INSERT INTO [dbo].[GL_M_RecurringJournal]
				([IDX_M_Company]
				,[IDX_M_Branch]
				,[RecurringCode]
				,[RecurringName]
			...
			VALUES
				...
				,CASE WHEN RTRIM(ISNULL(@AdjustLastPeriod,'')) = 'Y' THEN 'Y' ELSE 'N' END
				,@StartPeriod
				,NULLIF(RTRIM(ISNULL(@EndPeriod,'')),'')
				,@RecurringStatus
				,@RecordStatus
				,@UserID
				,GETDATE())

			SET @_IDX_M_RecurringJournal = SCOPE_IDENTITY()

			INSERT INTO @TableLog VALUES ('success', @_IDX_M_RecurringJournal, 'Data berhasil disimpan.')
		END
