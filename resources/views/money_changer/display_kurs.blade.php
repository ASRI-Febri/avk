...
<body>
    <div class="container-fluid mt-4">
        ...
        <table class="table table-borderless table-hover text-white mt-4" style="background: grey;">
            <thead>
                <tr>
                    <th>Negara</th>
                    <th>Mata Uang</th>
    ...
    <div class="ticker-container">
        <div class="ticker" id="tickerContent">
            @if($records)
                @foreach($records as $row)
                    <span class="{{ $row->IconFlag }}"></span><span>{{ $row->CurrencyID }} : Jual Rp {{ number_format($row->SellRate,2,'.',',') }} / Beli Rp {{ number_format($row->BuyRate,2,'.',',') }}</span>
                @endforeach
            @endif 
