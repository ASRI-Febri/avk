<table class="table table-bordered">
    ...
    @if($tax_detail)
        ...
        @foreach($tax_detail as $row)
            ...
            @php 
                $seq += 1;
                $url_delete = url('fm-sales-invoice-tax/delete/'.$row->IDX_T_SalesInvoiceTax);       
            @endphp
                ...
                <td class="text-center">
                    ...
                    <div class="input-group-prepend text-center">
                        <a id="btn-edit" class="btn btn-outline-secondary btn-sm" href="#" title="Edit"
                            onclick="editTax('{{$row->IDX_T_SalesInvoiceTax}}')">
                            <i class="fa fa-edit"></i>
                        </a>
                        <a id="btn-delete" class="btn btn-outline-danger btn-sm" href="#" title="Delete" 
                            onclick="deleteTax('{{$row->IDX_T_SalesInvoiceTax}}','{{$row->TaxDesc}}')">
        ...
        <tr>
            <td colspan="2"></td>
            <td colspan="2"><span><b>Total: {{ number_format($total_tax, 2, '.', ',') }}</b></span></td>
        </tr>
