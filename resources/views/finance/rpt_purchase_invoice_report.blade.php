...
@section('content') 
    ...
    <?php
        ...
        foreach ($records as $row):
            ...
            $total_paid += $row->PaymentAmount;
            $total_os += ($row->TotalInvoice - $row->PaymentAmount);
            //$total_os += $row->Outstanding;
            //$total_os_fp += $row->OutstandingFP;			
            //$total_amount += $row->SaldoAktual;

            $group1 = $row->PartnerName;
            
            if($row_number == 1){
                $group_name = $row->PartnerName;
                $group_prev_name = $row->PartnerName;
            } else {
                $group_name = $row->PartnerName;
            }
            ...
            $group_cr += $row->PaymentAmount;
            
            $group_os += ($row->TotalInvoice - $row->PaymentAmount);
            ...
            echo "<tr>\n";                
                ...
                */
                ...
                echo "<td class=\"$td_class text-right\">" . number_format($row->TotalInvoice - $row->PaymentAmount,2,'.',',') . "</td>\n"; 
